...
export async function getCurrentUser() {
    try {
        console.log("Attempting to retrieve current account...");
...
//Upload
export async function upload(category, book, episode) {
    console.log("Upload triggered:");

    // Create a file object (assuming the content is in a string format for simplicity)
    const fileContent = new Blob([episode], { type: 'text/plain' });
    const fileName = `${book}-${category}.txt`;

    try {
        // Use Appwrite Storage to create a file
        const response = await storage.createFile(
            config.storageId, // Replace with your storage ID
