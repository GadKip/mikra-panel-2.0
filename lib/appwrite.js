...
export async function upload(category, book, episode, file, client, userId) {
    ...
    try {
        ...
        // Create display filename (can contain Hebrew)
        const displayName = `${category}-${book}-${episode}.${fileExt}`;
        
        // Create valid fileId
        const fileId = `${ID.unique()}.${fileExt}`;

        console.log("Uploading file with the following details:");
        console.log("Storage ID:", config.storageId);
        console.log("File ID:", fileId);
        console.log("File:", file);

        // Create a new file object with the correct properties
        const fileObject = new File([file.fileBlob], displayName, { type: file.type });
...
export async function listFiles(client) {
    ...
    try {
        ...
        const groupedFiles = response.documents.reduce((acc, file) => {
            ...
            if (!acc[file.category][file.book]) {
                acc[file.category][file.book] = [];
            }
            
            // Add file to its group
            acc[file.category][file.book].push(file);
