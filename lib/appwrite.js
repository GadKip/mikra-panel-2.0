...
const account = new Account(null); // Initialize with null
...
export async function signIn(email, password, client) {
    ...
    try {
     // const activeSessions = await account.listSessions(client); // Skips listing existing sessions
      //  if (activeSessions.sessions.length > 0) {  // Skips clearing existing sessions
       //     await Promise.all(activeSessions.sessions.map(session => account.deleteSession(session.$id, client)));
       // }
